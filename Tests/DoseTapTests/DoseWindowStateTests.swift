...
final class DoseWindowStateTests: XCTestCase {
    ...
    func test_beforeWindow() {
        let anchor = Date()
        let now = makeDate(anchor, addMinutes: 120) // <150
    ...
    func test_completedTaken() {
        let anchor = Date()
        let d2 = makeDate(anchor, addMinutes: 170)
        let now = makeDate(anchor, addMinutes: 200)
        let calc = DoseWindowCalculator(now: { now })
    ...
    func test_snoozeLimitReached() {
        let anchor = Date()
        let now = makeDate(anchor, addMinutes: 170)
        var config = DoseWindowConfig()
        config.maxSnoozes = 2
