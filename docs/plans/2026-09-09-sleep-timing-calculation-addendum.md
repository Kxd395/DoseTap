...
**Second slice, bounded coverage (PR #16):** `SleepIntervalCoverage` supplies the common clip/union arithmetic, with separate sleep, awake, covered and unmeasured durations. The existing post-Dose-2 estimator delegates to it. An opt-in HealthKit query retains samples crossing an explicit window's edges and all eligible blocks inside it. No screen/export invokes that query; the legacy primary-episode selector remains separate. See [the bounded-coverage audit](../audit/2026-09-09/bounded-sleep-coverage.md).

**Third slice, reviewed-window entry:** the existing night-outcome diary stores optional user-reviewed bounds with session identity, review source/time and entry-zone/offset metadata. The editor requires explicit review of changed dates and retains correction history; existing questionnaires, final awakening and medication records stay separate. Collected-night exports and Studio reports carry the new fields. Entry zone does not establish the person's original location during sleep. See [the reviewed-window audit](../audit/2026-09-09/reviewed-night-window.md).

**Fourth slice, bounded source evidence:** the opt-in query retains original samples and origin metadata in its result. The versioned consensus resolver leaves overlapping contradictory observations unmeasured instead of selecting a stage by priority. Compatible unspecified-asleep and one detailed stage can contribute sleep; disagreeing detailed stages also remain unmeasured until reviewed. This policy is conservative bookkeeping, not evidence of device accuracy. Existing primary-episode charts and exports retain their prior semantics. See [the source-evidence audit](../audit/2026-09-09/sleep-evidence-conflicts.md).

**Fifth slice, local window assessment:** a read-only check now compares a saved window with current dose records, other reviewed windows and nap markers. Invalid/unreadable evidence, out-of-bounds doses, overlapping windows and incomplete/ambiguous overlapping naps remain explicit reasons to review. The editor and collected-night reports expose the same assessment meaning. No dates are rewritten, and the result does not establish provider coverage. See [the assessment audit](../audit/2026-09-09/reviewed-window-assessment.md).

The next integration must use these boundary checks together with the rich provider conflict result and recheck snapshots after asynchronous work. Query snapshots retain raw source evidence, but durable provider revision/deletion reconciliation and report provenance still need implementation. Never use a stale saved window or saved assessment after inputs change. Only then should History, Timeline, Dashboard and exports share treatment-night results. DOSETAP-57 still owns dose-linked onset/return markers and completed awakening counts. These prerequisite slices do not deliver those screens.
