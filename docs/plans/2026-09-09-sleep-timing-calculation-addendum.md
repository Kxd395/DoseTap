...
The next integration must retain conflicting raw provider evidence and resolve overlapping-session/nap conflicts, provider edits/deletions and corrected medication boundaries against the saved window. Never use a stale saved window without rechecking those inputs. Only then should History, Timeline, Dashboard and exports share treatment-night results. DOSETAP-57 still owns dose-linked onset/return markers and completed awakening counts. These prerequisite slices do not deliver those screens.
