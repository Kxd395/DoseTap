...
The next integration must resolve overlapping-session/nap conflicts and corrected medication boundaries against the saved window. Query snapshots now retain raw source evidence, but durable provider revision/deletion reconciliation and report provenance still need implementation. Never use a stale saved window without rechecking those inputs. Only then should History, Timeline, Dashboard and exports share treatment-night results. DOSETAP-57 still owns dose-linked onset/return markers and completed awakening counts. These prerequisite slices do not deliver those screens.
