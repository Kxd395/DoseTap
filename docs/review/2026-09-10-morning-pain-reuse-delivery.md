...
Cancelling review adds nothing. Using the foot pattern does not add back pain. An already-added area/side directs the user to Edit rather than reapplying that preference. Changing a different saved pattern to that occupied area/side is also blocked with visible Edit guidance. Ordinary editing preserves that morning's existing intensity/notes. Morning and History cannot update or forget preferences, and History does not offer current patterns as historical evidence. Existing bedtime behavior stays intact.
...
[PR #31 review](https://github.com/Kxd395/DoseTap/pull/31#discussion_r3985708989) identified a destination collision: a different saved pattern could be edited to an occupied morning area/side and replace the existing entry through the ordinary upsert. Saved review now checks the current draft destinations, disables Save and shows explicit Cancel/use Edit guidance. Ordinary Edit behavior is unchanged. The native normal-text journey adds a foot entry at zero, reviews the saved back pattern at seven, changes its area to the occupied foot destination, verifies the warning/disabled Save, cancels and verifies the original zero/numbness foot entry remains.

The extended normal-text journey passed on the review fix: 1 test, zero failures, 141.617 seconds; `/tmp/dosetap-morning-pain-ui-collision.xcresult`. The signed device artifact was refreshed again after this change (`/tmp/dosetap-morning-pain-device-collision.log`, build succeeded) and its signature verified. This extends the earlier first-open and largest-text evidence; it does not claim a new phone or VoiceOver run.
