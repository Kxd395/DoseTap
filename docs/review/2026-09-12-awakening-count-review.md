...
2. Extend the checked result with manual final wake and existing outcome provenance from its already revalidated outcome snapshot, then add the result to one reviewed-night card. Test source edits/removal and late responses. Obtain native text-size/VoiceOver and same-night owner/provider comparison; keep source samples and all logs inspectable.
