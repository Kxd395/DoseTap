...
The model extraction keeps `InsightBundleModels.swift` below its architecture ceiling without changing the DTO. Repository guards cover SSOT, documentation, Plane workflow (15 tests/80 assertions), dose writes, legacy safety, architecture, repository hygiene, tab ownership, companion targets and whitespace. They do not establish whole-project reader consistency.
