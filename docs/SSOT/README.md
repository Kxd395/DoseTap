...
Shipping app version observed in the Xcode project: 0.4.19 (build 37)
...
- `HealthKitService.fetchSleepCoverage(from:to:)` is an opt-in bounded query using overlapping samples, including samples that begin before the requested start. It now delegates to `fetchSleepEvidence(from:to:)` and the consensus policy above, excluding unresolved conflicts, in-bed and unknown categories from classified coverage. Existing primary-episode queries and displays are unchanged. No screen or export calls the new bounded query yet; durable source reconciliation and consumer integration remain open under DOSETAP-56.
