...
App version defined in this revision's Xcode project: 0.4.19 (67)
...
- Awakening counts: the [reviewed awakening-count contract](contracts/reviewed-awakening-counts.md) defines forthcoming completed-episode counts, incomplete evidence, Dose 2 subsets and denominators. It is not implemented count behavior. Current primary-episode `wakeCount`, WHOOP disturbances and quick-log counts retain separate meanings; see the [source/consumer audit](../review/2026-09-12-awakening-count-review.md). Builds 46/48 already provide reviewed dose/sleep durations and episode inspection.
...
occurrence through the existing medication ledger. Medication selection starts
...
Duplicate review compares absolute occurrences across date boundaries, separate
...
- The strict archive validator accepts omitted provider consent only for an explicitly local schema-2 export with a sessions array: `exportWarnings` must be a string array containing the exact element `Local snapshot only; provider enrichment was not fetched.` and structured provider payloads, availability, provenance, collected-night measurements and provider CSV values must not contradict that declaration. Explicit malformed consent remains invalid; other exports require the consent object with five boolean provider-state fields. An absent local consent object means not captured, not disabled or denied. This exception does not relax required metadata, questionnaire payload or ZIP validation (DOSETAP-13 / DC-12).
