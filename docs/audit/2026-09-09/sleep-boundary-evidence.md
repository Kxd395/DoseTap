...
- `DoseActionCoordinatorClockTests` and `MedicationMutationTransactionTests`, same simulator: 60 passed, including cancellation/backgrounding, stale confirmation and transaction rollback. `/tmp/dosetap56-medication.xcresult`. This regression pass does not close signed-device persistence or alarm acceptance.
