...
## Per-night batch failure isolation

The final review follow-up isolates failures while keeping the batch read snapshot. Previously, one malformed dose timestamp caused every selected assessment to become unavailable. The mixed-night red/green test now preserves two valid results and one verified absent window while flagging only the unreadable local dose snapshot. Shared window/nap failures still prevent checking saved windows; known absent windows remain missing. The complete targeted app rerun passed 126 tests (`/tmp/dosetap56-assessment-isolation-green.xcresult`). No stored data is repaired or skipped silently by the export serializer: its existing strict per-night reads can still reject an unreadable bundle rather than claim a complete export.
