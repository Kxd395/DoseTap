...
> Archived on 2026-09-02. Service paths and line counts in this snapshot are not maintained as current inventory.

## AlarmService

File: `ios/DoseTap/AlarmService.swift`
...
│  @Published reminderScheduled: Bool         │
│  @Published snoozeCount: Int                │
│  @Published isAlarmRinging: Bool            │
│  @Published lastSchedulingError: String?    │
│                                             │
│  Notification IDs:                          │
│  ├── dosetap_dose2_alarm                    │
...
| Method | Purpose |
| ------ | ------- |
| `scheduleDose2Alarm(at:dose1Time:)` | Transactionally schedule and verify the wake role group |
| `scheduleDose2Reminders(dose1Time:)` | Transactionally schedule and verify window-open + 15m/5m warnings |
| `snoozeAlarm(dose1Time:)` | Replace wake roles only after canonical phase validation |
| `reconcilePendingRequests(reason:dose1Time:forceReschedule:)` | Repair persisted intent against actual pending requests |
| `retryLastSchedule(dose1Time:)` | Explicit user retry of both groups |
| `cancelWakeAlarms()` | Cancel wake roles without erasing safety reminders |
| `cancelAllAlarms()` | Cancel both groups for completion or rollover |
| `stopRinging(acknowledge:)` | Stop local ringing state |
...
D1 at 10:30 PM, target = 165m
  ...
  01:15 AM ─── "dosetap_dose2_alarm" (target, 165m) ← RINGING
      │
  01:17 AM ─── followup_1 (target + 2m)
  01:19 AM ─── followup_2 (target + 4m)
  01:21 AM ─── followup_3 (target + 6m)
      │
...
The wake target is an absolute instant. Calendar triggers carry a fixed UTC
offset for that instant to avoid DST fall-back ambiguity, while the named
timezone is persisted as reconciliation provenance. A group is published as
scheduled only after its exact pending request set verifies. See
`docs/SSOT/alarm-scheduling.md` for the normative transaction, rollback,
reconciliation, and snooze-role contracts.
