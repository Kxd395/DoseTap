...
extension SettingsView {
    ...
    private func exportPreSleepSummary(from log: StoredPreSleepLog) -> InsightsPreSleepSummary {
        ...
        return InsightsPreSleepSummary(
            ...
            rawAnswersJson: encodedJSONString(from: answers),
    ...
    private func exportMorningSummary(from checkIn: StoredMorningCheckIn) -> InsightsMorningSummary {
        ...
        return InsightsMorningSummary(
            ...
            rawPhysicalSymptomsJson: checkIn.physicalSymptomsJson,
            rawRespiratorySymptomsJson: checkIn.respiratorySymptomsJson,
            rawSleepTherapyJson: checkIn.sleepTherapyJson,
            rawSleepEnvironmentJson: checkIn.sleepEnvironmentJson,
            rawStressContextJson: checkIn.stressContextJson,
            rawTimingContextJson: checkIn.timingContextJson,
    ...
    private func exportCheckInSubmission(from submission: StoredCheckInSubmission) -> InsightsCheckInSubmissionSummary {
        InsightsCheckInSubmissionSummary(
            ...
            checkInType: submission.checkInType.rawValue,
            questionnaireVersion: submission.questionnaireVersion,
            submittedAtUTC: submission.submittedAtUTC,
            localOffsetMinutes: submission.localOffsetMinutes,
            responsesJson: submission.responsesJson
        )
    }
    ...
    func studioWHOOPExportQueryRangeForTesting(sessionDates: [String]) -> (start: Date, end: Date)? {
        studioWHOOPExportQueryRange(for: sessionDates)
    }

    func studioWHOOPSessionDateForTesting(using repo: SessionRepository, summaryDate: Date) -> String {
    ...
    private func studioWHOOPExportQueryRange(for sessionDates: [String]) -> (start: Date, end: Date)? {
        let ranges = sessionDates.compactMap(studioQueryRange(for:))
        guard let start = ranges.map(\.start).min(),
              let end = ranges.map(\.end).max(),
              start < end else {
            return nil
        }
        return (start, end)
    ...
    private func studioWHOOPSessionDate(using repo: SessionRepository, summaryDate: Date) -> String {
        repo.sessionDateString(for: summaryDate)
    }
...
private struct InsightsCheckInSubmissionSummary: Codable {
    let id: String
