...
enum WakeLongAwakePeriod: String {
    ...
    init(rawSurveyValue: String) {
        switch rawSurveyValue.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() {
        case "<15m", "<15", "less_than_15":
            self = .lessThan15
        case "15-30m", "15-30":
            self = .fifteenTo30
        case "30-60m", "30-60":
            self = .thirtyTo60
        case "1h+", "1h", "1h+", "1hour+", "60+":
            self = .oneHourPlus
        default:
            self = .none
        }
...
final class MorningCheckInViewModelV2 {
    ...
    func applyLastWakeSurvey(from events: [StoredSleepEvent], excludingSessionDate: String) -> Bool {
        ...
        if let feeling = payload["feeling"] as? String {
            feelingNow = WakeFeelingNow(rawSurveyValue: feeling)
        }
        if let value = payload["sleep_quality"] as? Int {
            sleepQuality = value
        }
        if let value = payload["sleepiness_now"] as? Int {
            sleepinessNow = value
        }
        if let value = payload["pain_level"] as? Int {
            wakePainLevel = value
        }
        if let value = payload["pain_woke_user"] as? Bool {
            painWokeUser = value
        }
        if let value = payload["awakenings"] as? String {
            awakeningsCount = WakeAwakeningsCount(rawSurveyValue: value)
        }
        if let value = payload["long_awake"] as? String {
            longAwakePeriod = WakeLongAwakePeriod(rawSurveyValue: value)
        }
        if let value = payload["notes"] as? String {
            notes = value
        }
