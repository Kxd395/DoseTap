...
extension StudioWorkbookData {
    ...
    func fieldGuideSheet() -> WorkbookSheet {
        ...
        let contracts: [(String, String, String)] = [
            ("Workbook schema", "1", "Reporting snapshot from one finalized Studio archive; not a tested full-app restore. Table filters do not change fixed Overview summaries."),
        ...
        rows += Self.tableGrains.keys.sorted().map { name in [.text("Table mapping"), .text(name), .text("DoseTap" + name.filter { $0.isLetter || $0.isNumber }),
            .text("Named Excel table"), .blank, SW.text(Self.tableGrains[name]), .text("Empty sources contain zero observations"), .text("Workbook schema 1"), .blank, .blank] }
